...
const TRANSLATIONS = {
  vi: {
    ...
    nav_users: "Khách hàng",
    users_title: "Quản Lý Tài Khoản Khách Hàng",
    th_user_id: "Mã tài khoản",
    th_user_name: "Họ và tên",
    th_user_email: "Email",
    th_user_phone: "Số điện thoại",
    th_user_address: "Địa chỉ mặc định",
    th_user_created: "Ngày đăng ký",
    toast_load_users_fail: "Lỗi tải danh sách khách hàng.",
    toast_user_deleted: "Đã xóa tài khoản khách hàng thành công! 🌟",
    toast_user_delete_fail: "Lỗi xóa tài khoản khách hàng.",
    confirm_delete_user: "Bạn có chắc chắn muốn xóa tài khoản của khách hàng \"{name}\" không?",
    no_users_yet: "Chưa có khách hàng nào đăng ký."
  },
  en: {
    ...
    nav_users: "Customers",
    users_title: "Manage Registered Customers",
    th_user_id: "User ID",
    th_user_name: "Full Name",
    th_user_email: "Email",
    th_user_phone: "Phone",
    th_user_address: "Default Address",
    th_user_created: "Registered Date",
    toast_load_users_fail: "Failed to load customer list.",
    toast_user_deleted: "Customer account deleted successfully! 🌟",
    toast_user_delete_fail: "Failed to delete customer account.",
    confirm_delete_user: "Are you sure you want to delete account for customer \"{name}\"?",
    no_users_yet: "No registered customers yet."
  }
};
...
async function updateOrderStatus(orderId, newStatus) {
  try {
    ...
    await loadAnalytics(); // reload stats cards at the top
  } catch (err) {
...
// ==========================================
// TAB 6: Users / Customers Management
// ==========================================
async function loadUsers() {
  try {
    ...
    if (usersList.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="text-center" style="color:var(--text-muted); padding:3rem;">${t('no_users_yet')}</td></tr>`;
      return;
    }

    usersList.forEach(user => {
      const dateStr = user.createdAt ? new Date(user.createdAt).toLocaleString(activeLang === 'vi' ? 'vi-VN' : 'en-US') : 'N/A';
      ...
      tr.innerHTML = `
        ...
        <td style="color:var(--text-muted); font-size:0.75rem;">${dateStr}</td>
        <td class="text-center">
          <button class="btn-table-action btn-delete-action" onclick="deleteUser('${user.id}', '${escapeHTML(user.name).replace(/'/g, "\\'")}')" title="Delete Account">
    ...
    console.error(err);
    showToast(t('toast_load_users_fail'), 'danger');
  }
}
...
async function deleteUser(userId, userName) {
  if (confirm(t('confirm_delete_user', { name: userName }))) {
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to delete customer account');

      showToast(t('toast_user_deleted'), 'warning');
      await loadUsers(); // reload table
    } catch (err) {
      console.error(err);
      showToast(t('toast_user_delete_fail'), 'danger');
    }
  }
}
